...
class CSVProcessor:
    def __init__(self, csv_file_path: str, skiprows: int, batch_size: int, prob: float):
    ...
    def calculate_rmse(self, filled_df: pd.DataFrame):
        ...
        for start in range(0, len(self.df_2), self.batch_size):
            temp = self.df_2.iloc[start:start + self.batch_size]
            numeric_columns = temp.select_dtypes(include=np.number).columns
            rmse_values = [np.sqrt(mean_squared_error(temp[column], filled_df[column])) for column in numeric_columns]
...
async def process_csv():
    try:
        ...
        response = StreamingResponse(
            csv_processor.process_csv_in_batches(),
